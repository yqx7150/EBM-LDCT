...
from data import Imagenet, Cifar10, DSprites, Mnist, TFImagenetLoader,GetMRI,GetCT,GetCT_cirs
...
# for settling the numpy version problem
...
flags.DEFINE_string('dataset','mnist','dsprites, cifar10, imagenet (32x32) or imagenetfull (128x128)')
flags.DEFINE_integer('batch_size', 1, 'Size of inputs')
...
flags.DEFINE_integer('save_interval', 5000,'save outputs every so many batches')
flags.DEFINE_integer('test_interval', 5000,'evaluate outputs every so many batches')
...
flags.DEFINE_bool('ResNet128_model', False, 'Wider ResNet128 Network')
...
def decompress_x_mod(x_mod):
    x_mod = x_mod / 256 * FLAGS.rescale + np.random.uniform(0, 1 / 256 * FLAGS.rescale, x_mod.shape)
    return x_mod
...
def train(target_vars, saver, sess, logger, dataloader, resume_iter, logdir):
    ...
    for epoch in range(FLAGS.epoch_num):
        for data_corrupt, data, label in dataloader:
            ...
            #print(data.shape,data_corrupt.shape)
            #assert 0
            ...
            if FLAGS.replay_batch and (x_mod is not None):
                ...
                if len(replay_buffer) > FLAGS.batch_size:
                    ...
                    replay_mask = (np.random.uniform(0,FLAGS.rescale,FLAGS.batch_size) > 0.05)
            ...
            if itr % FLAGS.test_interval == 0 and hvd.rank() == 0 and FLAGS.dataset != '2d':
                ...
                for i, (im, t_im, actual_im_i) in enumerate(zip(orig_im[:8], try_im[:8], actual_im)):
                    
                    
                    shape = orig_im.shape[1:]
                    actual_im_i=np.squeeze(actual_im_i)
                ...
                if FLAGS.replay_batch and (x_mod is not None) and len(replay_buffer) > 0:
                    replay_batch = replay_buffer.sample(FLAGS.batch_size)
                    replay_batch = decompress_x_mod(replay_batch)
                    replay_mask = (np.random.uniform(0, 1, (FLAGS.batch_size)) > 0.05)
                    data_corrupt[replay_mask] = replay_batch[replay_mask]

                if FLAGS.dataset == 'cifar10' or FLAGS.dataset == 'imagenet' or FLAGS.dataset == 'imagenetfull' or FLAGS.dataset == 'fastMRI' or  FLAGS.dataset == 'CT':
                    #n = 128
                    n = 8
                    ...
                    if len(replay_buffer) > n:
                        ...
                        data_corrupt = np.random.uniform(0, FLAGS.rescale, (n, 128, 128, 3))
                    else:
                        data_corrupt = np.random.uniform(0, FLAGS.rescale, (n, 512, 512, 1))
                    ...
                    if FLAGS.dataset == 'cifar10':
                        ...
                        label = np.eye(1)[np.random.randint(0, 1, (n))]
                ###############################################################################
...
def main():
    ...
    if FLAGS.dataset == 'fastMRI':
        #dataset = Cifar10(augment=FLAGS.augment, rescale=FLAGS.rescale)
        #test_dataset = Cifar10(train=False, rescale=FLAGS.rescale)
        dataset = GetMRI(root = './input_data/train_64_3')
        test_dataset = GetMRI(root = './input_data/train_64_3')
        channel_num = 3
        
    elif FLAGS.dataset == 'CT':

        #dataset = GetCT(root = './train_data')
        #test_dataset = GetCT(root = './train_data')
        dataset = GetCT_cirs(root = './cirs_train_data')
        test_dataset = GetCT_cirs(root = './test_CIRS')
        channel_num = 1
    ...
    elif FLAGS.dataset == 'imagenet':############################### imagenet  32x32
    ...
    elif FLAGS.dataset == 'imagenetfull':############################### imagenetfull 128x128
        channel_num = 3
    ...
    for j in range(FLAGS.num_gpus):
        if FLAGS.model_cclass:
            ...
            x_split = tf.tile(
                tf.reshape(
                    X_SPLIT[j], (ind_batch_size, 1, 32, 32, 1)), (1, 10, 1, 1, 1))
            x_split = tf.reshape(x_split, (ind_batch_size * 10, 32, 32, 1))
        ...
        def langevin_step(counter, x_mod):
            ...
            x_grad, label_grad = tf.gradients(FLAGS.temperature * energy_noise, [x_mod, LABEL_SPLIT[j]])
        ...
        if FLAGS.train:
            if FLAGS.objective == 'logsumexp':
                ...
                loss_ml = FLAGS.ml_coeff * tf.nn.softplus(temp * (energy_pos - energy_neg))
            ...
            loss_total = loss_total + FLAGS.l2_coeff * (tf.reduce_mean(tf.square(energy_pos)) + tf.reduce_mean(tf.square((energy_neg))))
    ...
    saver = loader = tf.train.Saver(max_to_keep=None,keep_checkpoint_every_n_hours=10000)
