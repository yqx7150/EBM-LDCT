...
class Textures(Dataset):
    ...
    def __getitem__(self, index):
        ...
        im = np.array(im)[:32, :32] / 255############ gai 128
        im = im + np.random.uniform(-1 / 512, 1 / 512, im.shape)

        return im, im, label

# dataset = GetMRI(root= "./fastMRI_2941_python_128_6ch",augment=None)
class GetMRI(Dataset):
    def __init__(self,root):
        super().__init__()
        labels = [0 for _ in range(64000)]
        self.data_names = np.array([root+"/"+x  for x in os.listdir(root)])
        self.labels = labels
        self.one_hot_map = np.eye(1)

    def __len__(self):
        return len(self.data_names)

    def __getitem__(self, index):
        siat=loadmat(self.data_names[index])['Img2']
        #siat=np.array(siat,dtype=np.float32)
        siat_data=np.zeros((64, 64, 3))
        siat_data=np.array(siat,dtype=np.float32)

        label = self.labels[index]
        label = self.one_hot_map[label]

        image_size = 64
        siat_data = siat_data + np.random.uniform(-1 / 512, 1 / 512, siat_data.shape)
        np.random.seed((index + int(time.time() * 1e7)) % 2**32)

        if FLAGS.datasource == 'default':
            im_corrupt = siat_data + 0.3 * np.random.randn(image_size, image_size, 3)
        elif FLAGS.datasource == 'random':
            im_corrupt = np.random.uniform(
                0.0, 1.0, (image_size, image_size, 3))

        return im_corrupt, siat_data, label
        
class GetCImageNetData(Dataset):
    def __init__(self,root):
        super().__init__()
        labels = [0 for _ in range(38400)]
        self.data_names = np.array([root+"/"+x  for x in os.listdir(root)])
        self.labels = labels
        self.one_hot_map = np.eye(1)
        self.rescale=1.0
    ...
    def __getitem__(self, index):
        ...
        im=mpimg.imread(self.data_names[index]) #(0,1)
        #im=io.imread(self.data_names[index]) #(0,255)
        im=np.array(im,dtype=np.float32)

        im_data=np.zeros((64, 64, 3))
        ...
        im_data = im_data * 255 / 256
        im_data = im_data + np.random.uniform(0 , self.rescale * 1 / 256., im.shape)
        np.random.seed((index + int(time.time() * 1e7)) % 2**32)
        

        if FLAGS.datasource == 'default':
            im_corrupt = im_data + 0.3 * np.random.randn(image_size, image_size, 3)
        elif FLAGS.datasource == 'random':
            im_corrupt = np.random.uniform(0.0, self.rescale, (image_size, image_size, 3))

        return im_corrupt, im_data, label
class GetCT(Dataset):
    def __init__(self,root):
        super().__init__()
        labels = [0 for _ in range(2961)]
        self.data_names = np.array([root+"/"+x  for x in os.listdir(root)])
        self.labels = labels
        self.one_hot_map = np.eye(1)
    ...
    def __getitem__(self, index):
        ...
        #siat=loadmat(self.data_names[index])
        #siat=np.array(siat,dtype=np.float32)
        CT_data=np.zeros((512, 512, 1))
        CT_data=np.array(CT_data,dtype=np.float32)

        CT_data[:,:,:1] = data_array
      
        label = self.labels[index]
        label = self.one_hot_map[label]

        image_size = 512
        CT_data = CT_data + np.random.uniform(0, 1 / 512, CT_data.shape)
        np.random.seed((index + int(time.time() * 1e7)) % 2**32)

        if FLAGS.datasource == 'default':
            im_corrupt = CT_data + 0.3 * np.random.randn(image_size, image_size, 1)
        elif FLAGS.datasource == 'random':
            im_corrupt = np.random.uniform(
                0.0, 1.0, (image_size, image_size, 1))

        return im_corrupt, CT_data, label
class GetCT_cirs(Dataset):
    def __init__(self,root):
        super().__init__()
        labels = [0 for _ in range(1785)]
        imas_path = glob.glob('./cirs_train_data/*.dcm')

        self.data_names = imas_path
        self.labels = labels
        self.one_hot_map = np.eye(1)
    ...
    def __getitem__(self, index):      
        dataset_input = dicom.read_file(self.data_names[index])
        input_ = dataset_input.pixel_array.astype(np.float32)    ## 读取像素范围 0-4096
        input_ -= 1024
        input_[input_<-1024]=-1024
        input_ += 1024
        RescaleSlope = dataset_input.RescaleSlope
        RescaleIntercept = dataset_input.RescaleIntercept
        CT_input = input_ * RescaleSlope + RescaleIntercept
        
        data_array = (CT_input-np.min(CT_input))/(np.max(CT_input)-np.min(CT_input)) ### 0-1
        data_array = np.expand_dims(data_array,2)
...
class GetCT_3c(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #siat=loadmat(self.data_names[index])
        #siat=np.array(siat,dtype=np.float32)
        CT_data=np.zeros((512, 512, 3))
        CT_data=np.array(CT_data,dtype=np.float32)
        
        CT_data[:,:,0] = data_array
        CT_data[:,:,1] = data_array
        CT_data[:,:,2] = data_array
        label = self.labels[index]
        label = self.one_hot_map[label]
        ...
        if FLAGS.datasource == 'default':
            im_corrupt = CT_data + 0.3 * np.random.randn(image_size, image_size, 3)
        elif FLAGS.datasource == 'random':
            im_corrupt = np.random.uniform(
                0.0, 1.0, (image_size, image_size, 3))
